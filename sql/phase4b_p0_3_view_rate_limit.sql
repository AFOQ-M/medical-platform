...
--
-- ⚠️ تصحيح توثيقي (P1-Final، M3): القيمة الحية الفعلية للتهدئة هي
-- **10 دقائق**، وليست 30 دقيقة كما كان موثَّقًا سابقًا في هذا الملف.
-- المصدر الرسمي للقيمة الحية هو
-- sql/phase4b_p0_3_p0_4_digest_schema_fix.sql (hotfix لاحق أعاد تعريف
-- increment_resource_view عبر CREATE OR REPLACE بنفس التوقيع، وغيّر
-- فقط تأهيل digest() ليكون extensions.digest؛ ولم يُغيَّر قصدًا أي
-- سلوك آخر، لكن قيمة v_cooldown فيه كانت أصلاً 10 دقائق منذ ذلك
-- الملف). تحقّقنا حيًا في هذه الجلسة (عبر pg_get_functiondef) أن
-- الدالة الحية على القاعدة تستخدم فعليًا 10 دقائق. هذا الملف (النسخة
-- الأقدم) لا يزال يوثّق 30 دقيقة أدناه لأسباب تاريخية فقط — لا تُعِد
-- تطبيقه كما هو على أي قاعدة حية، فهو ليس النسخة النهائية المطبَّقة.
-- القرار النهائي والمعتمد هو 10 دقائق.
--
-- المشكلة:
...
--   - نافذة تهدئة (cooldown) ثابتة منذ آخر مشاهدة فعلية محسوبة لنفس
--     الزوج. كانت موثَّقة هنا أصلاً كـ30 دقيقة (رقم مقترح وقت كتابة
--     هذا الملف)، لكن القيمة النهائية المعتمدة والحية فعليًا هي
--     **10 دقائق** (انظر ملاحظة التصحيح أعلى الملف وsql/phase4b_p0_3_p0_4_digest_schema_fix.sql).
...
as $$
declare
  v_cooldown constant interval := interval '30 minutes'; -- ⚠️ تاريخي/غير مطابق للحي: القيمة الحية الفعلية 10 دقائق (انظر ملاحظة أعلى الملف) — لا تُطبَّق هذه النسخة كما هي
  v_raw_ip   text;
  v_rate_key text;
...
comment on function increment_resource_view(uuid) is
  'يزيد view_count لمورد منشور بحد أقصى مرة واحدة كل 10 دقائق لكل زوج (هاش مصدر، مورد). P0-3. القيمة الحية النهائية 10 دقائق، انظر sql/phase4b_p0_3_p0_4_digest_schema_fix.sql — هذا الملف تاريخي، لا يُطبَّق كما هو.';
